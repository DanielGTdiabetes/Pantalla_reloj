...
const Weather = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeWeather(setWeather, 5);
    return unsubscribe;
  }, []);
    ...
    >
      <div className="flex items-center gap-6 text-left">
        ...
        <div>
          ...
          <p className="mt-3 flex gap-6 text-sm text-slate-200/80">
            <span>Humedad {weather.humidity}%</span>
            <span>Prec. {weather.precipProb}%</span>
            <span>Actualizado {new Date(weather.updatedAt).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</span>
          </p>
        </div>
      </div>
    </section>
